...
- 修复：微信收到的图片在本地只有加密 `.dat`，此前 `get_media` 取不到数据，模型完全看不到图片
- 收到图片且接口暂时拿不到数据时，插件会定位会话里可见的图片气泡并逐点打开，促使微信写出未加密副本后自动重新取图
- 依赖 agent-wechat 同步改动：优先读取微信自己渲染出的 `temp/ImageUtils/<md5>.<ext>`，不再强依赖图片解密密钥
- 图片解码失败时保持原有降级行为（不发送错误消息、不影响文本回复）

## 0.3.25 - 2026-09-12

- 修复：平台适配器的 `config_metadata` 之前写成了 `{"en-US": {...}}` + `label`/`field_type` 的 3.x 风格，而 AstrBot 4.x 会把顶层键当成字段名塞进表单，导致 WebUI 平台配置里一个字段都显示不出来
- 改为扁平结构 + `description`/`type`/`hint`（与内置适配器一致），现在平台编辑弹窗会显示服务地址、访问令牌、X11 发送相关选项以及「群聊免@成员」
- `group_mention_free_senders` 表单类型改为 `string`（逗号/换行分隔），仍兼容列表写法
## 0.3.24 - 2026-09-12

- 文档：在 `group_mention_free_senders` 配置项的 WebUI 说明里补充「未 @ 的群消息不会被丢弃，仍会作为群聊上下文进入 AstrBot」，并提示需要开启 `provider_ltm_settings.group_icl_enable`
- 无行为变更，仅说明补充与版本号更新

## 0.3.23 - 2026-09-12

- 修正 0.3.22 的行为：群里未 @ 机器人的消息不再被插件丢弃，改为照常进入 AstrBot
- 这类消息不注入 `At(self_id)`，因此不会触发大模型回复，但会流经 AstrBot，群聊上下文照常可用
- 依据：AstrBot 的 `process_stage` 只在 `is_at_or_wake_command` 为真时才调用大模型，
  而该标记只由「被 @ / 唤醒前缀 / 私聊」设置，插件 handler 放行只影响事件是否被 stop
- 顺带修复：只有真正 @ 了机器人时才去掉开头的 @ 提及，其他消息保留原文，便于上下文还原

## 0.3.22 - 2026-09-12

- 群聊触发策略改为「需要 @ 机器人」，不再无条件唤醒
- 新增配置项 `group_mention_free_senders`：名单里的成员（填 wxid）在群里发言无需 @ 即可触发；其他人仍然必须 @
- 名单为空时，群里所有人都必须 @ 机器人才会回复
- 群里既没 @ 机器人、又不在免@名单里的消息不注入 `At(self_id)`，因此不会触发大模型
- 修复：`is_leading_self_mention` 兜底判断之前被无条件唤醒的逻辑覆盖，属于死代码，现已生效
- `normalize_allowlist` 现在同时接受列表和逗号/分号/空白分隔的字符串，便于在 WebUI 里直接手填
- 新增 `should_wake_group_message` 纯函数与对应单测

## 0.3.21 - 2026-09-12

- 优化 X11 发送通道速度：单条文本发送从约 `2.66s` 降到约 `1.25s`
- 去掉粘贴后写死的 `0.8s` 等待，改为轮询发送按钮状态确认，贴不进去会直接报错
- 合并「输入框是否为空」校验与焦点切换，减少一次无障碍树请求
- 缓存会话显示名称，避免每次发送都重新解析
- 无障碍树轮询间隔从 `400ms` 降到 `120ms`，等待次数相应放大以保持总重试预算
- 无控制流行为变更，补充验证：连续 8 条消息按序送达，无重复、无串消息
