...
- 新增：微信「表情」消息（local_type 47）现在也会作为图片送给模型，不再只把表情消息里的 CDN 链接当正文发出去
- 表情消息的 CDN 链接对模型没有意义，插件改为丢弃链接、走取媒体流程；取不到时回退成 `[表情]` 占位
- 依赖 agent-wechat 同步改动：表情优先读本地缓存，读不到就下载 CDN 上的真实 GIF/PNG
